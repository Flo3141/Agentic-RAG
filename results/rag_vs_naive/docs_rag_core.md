# Dokumentation für core
> Generiert mit Standard RAG (AST + Vector Search)
...
**Summary**
Base exception class for all calculator errors. This class is never raised directly; it serves as the root of the calculator's exception hierarchy.
...
**Raises**
- None: This class is never raised directly.

**Examples**

```python
# Define a subclass of CalculatorError
class CalculationLimitError(CalculatorError):
    """Exception raised when calculation exceeds allowed limits."""
    pass

# Raising CalculationLimitError in user code
try:
    raise CalculationLimitError("Numbers exceed maximum allowed limit of 1,000,000")
except CalculatorError as e:
    print(f"Calculator error: {e}")
```

**See also**
- `CalculationLimitError`: A subclass of `CalculatorError` that is raised when calculation limits are exceeded.
...
**Summary**  
A custom exception class raised when a user-requested precision value exceeds the calculator's internal precision limits (e.g., 100 decimal places). This ensures calculations adhere to safety constraints to prevent numerical instability, overflow, or excessive computational overhead.

**Parameters**  
- `message` (str): Optional error message (default: `f"Requested precision {precision} exceeds maximum allowed {MAX_PRECISION}"`).  
- `cause` (Exception): Optional root cause exception (e.g., `ValueError` from invalid input).

**Returns**  
- `None`: The `PrecisionError` class does not return values. When raised, it interrupts execution and propagates up the call stack.

**Raises**  
- `PrecisionError`: This exception is raised by the calculator's precision validation logic when the requested precision exceeds the maximum allowed precision (e.g., `precision > MAX_PRECISION`).

**Examples**  
```python
from calculator.core import CalculatorError, PrecisionError

class Calculator:
    MAX_PRECISION = 100  # Example max precision

    def __init__(self):
        self._precision = 100  # Default precision

    def calculate(self, a, b, precision=100):
        # Simulate precision check (real implementation uses _check_limits)
...
try:
    result = calculator.calculate(1.0, 2.0, precision=200)  # Triggers PrecisionError
except PrecisionError as e:
    print(f"Precision error: {e}")  # Output: "Precision error: Requested precision 200 exceeds max 100"
```

**See also**  
- `CalculatorError`: Base exception class for all calculator-related errors.  
- `MAX_PRECISION`: The maximum allowed precision (e.g., 100 decimal places) defined in the calculator context.
